...
namespace asmio {
	...
	struct __attribute__((__packed__)) ElfSegmentHeader {
		...
		uint64_t vaddr;      ///< Virtual address at which the segment should reside in memory
		uint64_t paddr;      ///< On systems for which physical addressing is relevant, physical address at which the segment should reside in memory
		uint64_t filesz;     ///< Number of bytes in the file image of the segment
		uint64_t memsz;      ///< Number of bytes in the memory image of the segment
		uint64_t align;      ///< Alignment should be a positive, integral power of 2, and virtual_address should equal offset, modulo alignment
	};
